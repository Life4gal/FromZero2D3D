...
//
// 代码宏
// TODO 这些可恶的的胶水宏居然没有什么有效的方法让它消失,实在是令人不爽
//
...
namespace
{
	...
	{\
		...
		{\
			auto it = m_pImpl->m_##FullShaderType##s.find(pDesc->name##ShaderType##);\
			if (it != m_pImpl->m_##FullShaderType##s.end())\
			{\
	...
	{\
		for (auto& it : m_pImpl->m_##FullShaderType##s)\
		{\
...
struct CBufferData : CBufferBase
{
	...
	CBufferData(const LPCSTR name, const UINT startSlot, const UINT byteWidth, BYTE* initData = nullptr) :
		CBufferBase(), pData(new BYTE[byteWidth]{}), cbufferName(name), startSlot(startSlot),
		byteWidth(byteWidth)
	{
		if (initData)
...
struct ConstantBufferVariable : public IEffectConstantBufferVariable
{
	...
	ConstantBufferVariable(const ConstantBufferVariable& other) = delete;
	ConstantBufferVariable& operator=(const ConstantBufferVariable& other) = delete;
	...
	ConstantBufferVariable(const UINT offset, const UINT size, CBufferData* pData)
	...
	void SetUIntMatrix(const UINT rows, const UINT cols, const UINT* noPadData) override
	{
		SetMatrixInBytes(rows, cols, reinterpret_cast<const BYTE*>(noPadData));
	}

	void SetSIntMatrix(const UINT rows, const UINT cols, const INT* noPadData) override
	{
		SetMatrixInBytes(rows, cols, reinterpret_cast<const BYTE*>(noPadData));
	}

	void SetFloatMatrix(const UINT rows, const UINT cols, const FLOAT* noPadData) override
	{
		SetMatrixInBytes(rows, cols, reinterpret_cast<const BYTE*>(noPadData));
	}

	void SetRaw(const void* data, const UINT byteOffset = 0, UINT byteCount = 0xFFFFFFFF) override
	{
		...
		// 仅当值不同时更新
		if (memcmp(pCBufferData->pData.get() + startByteOffset + byteOffset, data, byteCount) != 0)
		{
	...
	HRESULT GetRaw(void* pOutput, const UINT byteOffset = 0, const UINT byteCount = 0xFFFFFFFF) override
	{
	...
	void SetMatrixInBytes(const UINT rows, const UINT cols, const BYTE* noPadData) const
	{
		// 仅允许1x1到4x4
		...
		{
			const UINT rowPitch = sizeof(DWORD) * cols < remainBytes ? sizeof(DWORD) * cols : remainBytes;
			// 仅当值不同时更新
			if (memcmp(pData, noPadData, rowPitch) != 0)
			{
...
{
	EffectPass(std::unordered_map<UINT, CBufferData>& cBuffers,
		...
		: cBuffers(cBuffers), shaderResources(shaderResources),
		samplers(samplers), rwResources(rwResources)
	{
	}
...
HRESULT EffectHelper::Impl::UpdateShaderReflection(LPCSTR name, ID3D11Device* device, ID3D11ShaderReflection* pShaderReflection, UINT shaderFlag)
{
	...
	HRESULT hr = pShaderReflection->GetDesc(&sd);
	if (FAILED(hr))
		return hr;
	...
	for (UINT i = 0;; ++i)
	{
		...
		if (sibDesc.Type == D3D_SIT_CBUFFER)
		{
			...
			const bool isParam = !strcmp(sibDesc.Name, "$Params");
...
HRESULT EffectHelper::AddShader(const LPCSTR name, ID3D11Device* device, ID3DBlob* blob) const
{
	if (!name)
		return E_INVALIDARG;
	...
	HRESULT hr = D3DReflect(blob->GetBufferPointer(), blob->GetBufferSize(), __uuidof(ID3D11ShaderReflection),
	...
	const UINT shaderFlag = static_cast<ShaderFlag>(1 << D3D11_SHVER_GET_TYPE(sd.Version));

	// 创建着色器
	hr = m_pImpl->CreateShaderFromBlob(name, device, shaderFlag, blob);
	if (FAILED(hr))
		return hr;

	// 建立着色器反射
	return m_pImpl->UpdateShaderReflection(name, device, pShaderReflection.Get(), shaderFlag);
}

HRESULT EffectHelper::AddGeometryShaderWithStreamOutput(const LPCSTR name, ID3D11Device* device, ID3D11GeometryShader* gsWithSO, ID3DBlob* blob) const
{
	if (!name || !gsWithSO)
		return E_INVALIDARG;
	...
	// 着色器反射
	ComPtr<ID3D11ShaderReflection> pShaderReflection;
	const HRESULT hr = D3DReflect(blob->GetBufferPointer(), blob->GetBufferSize(), __uuidof(ID3D11ShaderReflection),
	                              reinterpret_cast<void**>(pShaderReflection.GetAddressOf()));
	if (FAILED(hr))
		return hr;
	...
	m_pImpl->m_GeometryShaders[name] = std::make_shared<GeometryShaderInfo>();
	m_pImpl->m_GeometryShaders[name]->pGS = gsWithSO;

	// 建立着色器反射
	return m_pImpl->UpdateShaderReflection(name, device, pShaderReflection.Get(), shaderFlag);
}

void EffectHelper::Clear() const
{
	m_pImpl->Clear();
}

HRESULT EffectHelper::AddEffectPass(const LPCSTR effectPassName, ID3D11Device* device, EffectPassDesc* pDesc) const
{
	if (!pDesc || !effectPassName)
		return E_INVALIDARG;

	// 不允许重复添加
	const auto it = m_pImpl->m_EffectPasses.find(effectPassName);
	if (it != m_pImpl->m_EffectPasses.end())
		return ERROR_OBJECT_NAME_EXISTS;

	auto pEffectPass = m_pImpl->m_EffectPasses[effectPassName] = 
		std::make_shared<EffectPass>(m_pImpl->m_CBuffers, m_pImpl->m_ShaderResources, m_pImpl->m_Samplers, m_pImpl->m_RWResources);
...
std::shared_ptr<IEffectPass> EffectHelper::GetEffectPass(const LPCSTR effectPassName) const
{
	const auto it = m_pImpl->m_EffectPasses.find(effectPassName);
	if (it != m_pImpl->m_EffectPasses.end())
		return it->second;
...
std::shared_ptr<IEffectConstantBufferVariable> EffectHelper::GetConstantBufferVariable(const LPCSTR name) const
{
	const auto it = m_pImpl->m_ConstantBufferVariables.find(name);
	if (it != m_pImpl->m_ConstantBufferVariables.end())
		return it->second;
	
	return nullptr;
}

void EffectHelper::SetSamplerStateBySlot(const UINT slot, ID3D11SamplerState* samplerState) const
{
	auto it = m_pImpl->m_Samplers.find(slot);
	if (it != m_pImpl->m_Samplers.end())
		it->second.pSS = samplerState;
}

void EffectHelper::SetSamplerStateByName(const LPCSTR name, ID3D11SamplerState* samplerState) const
{
	auto it = std::find_if(m_pImpl->m_Samplers.begin(), m_pImpl->m_Samplers.end(),
...
void EffectHelper::SetShaderResourceBySlot(const UINT slot, ID3D11ShaderResourceView* srv) const
{
	auto it = m_pImpl->m_ShaderResources.find(slot);
	if (it != m_pImpl->m_ShaderResources.end())
		it->second.pSRV = srv;
}

void EffectHelper::SetShaderResourceByName(const LPCSTR name, ID3D11ShaderResourceView* srv) const
{
	auto it = std::find_if(m_pImpl->m_ShaderResources.begin(), m_pImpl->m_ShaderResources.end(),
...
void EffectHelper::SetUnorderedAccessBySlot(const UINT slot, ID3D11UnorderedAccessView* uav, UINT* pUAVInitialCount) const
{
	auto it = m_pImpl->m_RWResources.find(slot);
	if (it != m_pImpl->m_RWResources.end())
		it->second.pUAV = uav;
}

void EffectHelper::SetUnorderedAccessByName(const LPCSTR name, ID3D11UnorderedAccessView* uav, UINT* pUAVInitialCount) const
{
	auto it = std::find_if(m_pImpl->m_RWResources.begin(), m_pImpl->m_RWResources.end(),
...
void EffectHelper::SetDebugObjectName(const std::string& name) const
{
#if (defined(DEBUG) || defined(_DEBUG)) && (GRAPHICS_DEBUGGER_OBJECT_NAME)
	for (auto& it : m_pImpl->m_CBuffers)
	{
		std::string cBufferName = name + "." + it.second.cbufferName;
		it.second.cBuffer->SetPrivateData(WKPDID_D3DDebugObjectName, static_cast<UINT>(cBufferName.size()), cBufferName.c_str());
	}
